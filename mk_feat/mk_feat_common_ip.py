...
fday = df['day'].min()
lday = df['day'].max()
while len(df[df.day == fday]) < 1000:
    fday += 1
while len(df[df.day == lday]) < 1000:
    lday -= 1
print('!'*100)
print('fday:', fday)
print('lday:', lday)
...
com_df = df[flt_ip]
# print(com_df[['ip', 'day', 'is_attributed']])
# print(com_df[['ip', 'day', 'is_attributed']].groupby(by=['ip', 'day'])[['is_attributed']].count())
# sys.exit(0)
...
def dump_pct_com_ip(pct):
    ...
    dummy = 'is_attributed'
    cols = ['ip', 'day']
    cols_with_dummy = ['ip', 'day', 'is_attributed']

    # gp_ip_day = com_df[cols_with_dummy].groupby(by=cols)[[dummy]].count().reset_index().rename(index=str, columns={dummy: 'count'})
    gp_ip_day = com_df[cols_with_dummy].groupby(by=cols)[[dummy]].count()
    gp_ip_day = gp_ip_day.reset_index()
    gp_ip_day = gp_ip_day.rename(index=str, columns={dummy: 'count'})
