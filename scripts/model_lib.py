...
def get_params(params_str):
    ...
    if len(pvals) != len(names):
        ...
        sys.exit()
    params = dict(zip(names, pvals))  # pair parameters and their values as dict
    return params
...
def LGBM_helper(_X_tr, _X_va, _X_te, predictors, cat_feats, params, seed=2018):
    ...
    bst = lgb.train(params, lgtrain, valid_sets=valid_sets, valid_names=valid_names, evals_result=evals_results, num_boost_round=2000, early_stopping_rounds=100, verbose_eval=10, )
